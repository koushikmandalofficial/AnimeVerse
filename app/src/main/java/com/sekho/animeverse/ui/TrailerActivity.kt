...
class TrailerActivity : AppCompatActivity() {
    ...
    private fun fetchDetailsData(animeApiService: AnimeApiService) {
        activityScope.launch {
            try {
                val response = withContext(Dispatchers.IO) {
                    animeApiService.getAnimeDetails(contentId)
                }

                val animeDetails = response.data
                ...
                animeDetails.let { details ->
                    ...
                    Glide.with(this@TrailerActivity)
                        .load(bannerUrl)
                        .error(R.drawable.land_load)
                        .placeholder(R.drawable.land_load)
    ...
    private fun fetchProducersData(animeApiService: AnimeApiService) {
        activityScope.launch {
            try {
                val response = withContext(Dispatchers.IO) {
                    animeApiService.getAnimeProducers(contentId)
                }
